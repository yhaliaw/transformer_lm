...
if [ "$1" == "train" ]; then
  ...
  python3 "$ROOT"/train.py --data data/wikitext-103 --worker 1 \
    ...
    --layer-dropout 0.25 \
    --cuda --fp16 \
    --run-name lm_shared_layer_4_4_4_4_layerdrop_0.25 \
    "${@:2}"
elif [ "$1" == "eval" ]; then
  ...
  python3 "$ROOT"/eval.py --data data/wikitext-103 --worker 1 \
    ...
    --adaptive-softmax-dropout 0.2 \
    --cuda --fp16 \
    --checkpoint workspace/wiki103/lm_shared_layer_4_4_4_4_layerdrop_0.25/checkpoint/checkpoint_best.pt \
    "${@:2}"
else
